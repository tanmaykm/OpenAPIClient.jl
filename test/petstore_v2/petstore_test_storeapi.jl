...
function test(uri)
    ...
    @test_throws OpenAPIClient.ValidationException Order(; id=5, petId=10, quantity=2, shipDate=DateTime(2017, 03, 12), status="invalid_status", complete=false)
    ...
    @test_throws OpenAPIClient.ValidationException getOrderById(api, 0)
    ...
    @test_throws OpenAPIClient.ValidationException getOrderById(api, response_channel, 0)
    @sync begin
        @async begin
